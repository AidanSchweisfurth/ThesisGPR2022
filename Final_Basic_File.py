...
########################################
# Basic Regression File
########################################
# this file optimizes a non-warped system using SCG
...
def fullPlot(mean, quantiles, name="Basic", CCC=0, num = 0):
...
def extractFeatures(filename, output):
    #extract features from filename.wav and save in file to output
...
def loadFile(X, Y, name='tester.npy'):
    #load hyperparameters from name
...
#system does ~600 iterations per loop then pasues to save data and start again
...
while (m.optimize(optimizer = 'SCG', messages=True, max_iters=200).status == 'maxiter exceeded'):
    ...
    f = open(outputDir + 'Basic' + str(x) + '.txt', mode='w')
    f.write("Final CCC is " + CCC)
    f.write("\nValues:")
    f.write('\n' + str(m))
    f.write("\nLength Scales:")
    ...
    saveModel(m, name=(outputDir + 'Basic' + str(x) + '.npy'))
...
fullPlot(mean, quantiles, name=('Basic' + str(x)), num = x, CCC=CCC)
