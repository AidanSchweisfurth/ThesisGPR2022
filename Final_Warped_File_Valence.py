...
def extractRating(file):
    ...
    f = open('ratings_individual/valence/' + file, 'rt')
    data = csv.reader(f)
    for row in data:
        x = row[0].split(';')
        val = x[1:]
...
def getInputs(file='train_'):
    ...
    np.savetxt('features/Full Feature/ratings_train_val.csv', ratings.astype(float), delimiter=",")
...
def fullPlot(mean, quantiles, name="Plot.png", CCC=0, num = 0):
    for i in range(1, 10):
        ...
        fig.savefig('WarpedFinalVal/' + name + '_Dev_' + str(i) + '.png', dpi=500)
        plt.close()
        #plt.show()
    f = open('WarpedFinalVal/Warped' + str(num) + '.txt', mode='w')
...
Y = np.genfromtxt('features/Full Feature/ratings_train_val.csv', delimiter=',')
#X, Y = getInputs(file = "test_")
# print(X.shape)
# print(Y.shape)
#m = loadFile(X, Y, 'WarpedFinal/Warped209.npy')
...
testRatings = np.genfromtxt('features/Full Feature/ratings_dev_val.csv', delimiter=',')
...
while (m.optimize(optimizer = 'SCG', messages=True, max_iters=200).status == 'maxiter exceeded'):
    ...
    f = open('WarpedFinalVal/Warped' + str(x) + '.txt', mode='w')
    f.write("Final CCC is " + CCC)
    f.write("\nValues:")
    f.write('\n' + str(m))
    f.write("\nLength Scales:")
    ...
    f.write('\n' + str(m.inducing_inputs))
    f.close()
    saveModel(m, name=('WarpedFinalVal/Warped' + str(x) + '.npy'))
    x += 1
