...
from GPy.util.linalg import mdot, jitchol, backsub_both_sides, tdot, dtrtrs, dtrtri, dpotri, dpotrs, symmetrify
...
class NewInference(LatentFunctionInference):
    ...
    def to_dict(self):
        input_dict = super(NewInference, self)._save_to_input_dict()
    ...
    def inference(self, kern, X, Z, likelihood, Y, Y_metadata=None, mean_function=None, precision=None, Lm=None, dL_dKmm=None, psi0=None, psi1=None, psi2=None, Z_tilde=None):
        ...
        self.calcer(kern, X, Z, likelihood, np.array([Y[:, 1]]).T, Y_metadata=Y_metadata, mean_function=mean_function)
        marg2, grad2, woodbury_inv, vec3, Kmm, Lm = \
        self.calcer(kern, X, Z, likelihood, np.array([Y[:, 2]]).T, Y_metadata=Y_metadata, mean_function=mean_function)
        marg3, grad3, woodbury_inv, vec4, Kmm, Lm = \
        self.calcer(kern, X, Z, likelihood, np.array([Y[:, 3]]).T, Y_metadata=Y_metadata, mean_function=mean_function)
        marg4, grad4, woodbury_inv, vec5, Kmm, Lm = \
        ...
        grad_dict['dL_dthetaL'] = grad_dict['dL_dthetaL'] + grad1['dL_dthetaL'] + grad2['dL_dthetaL'] + grad3['dL_dthetaL'] + grad4['dL_dthetaL'] + grad5['dL_dthetaL']
        grad_dict['dL_dm'] = grad_dict['dL_dm'] + grad1['dL_dm'] + grad2['dL_dm'] + grad3['dL_dm'] + grad4['dL_dm'] + grad5['dL_dm']
    ...
    def calcer(self, kern, X, Z, likelihood, Y, Y_metadata=None, mean_function=None, precision=None, Lm=None, dL_dKmm=None, psi0=None, psi1=None, psi2=None, Z_tilde=None):
        ...
        return log_marginal, grad_dict, woodbury_inv, woodbury_vector, Kmm, Lm
